...
async function fetchJson(url) {
  const res = await fetch(url, {
    headers: {
      "user-agent": "mem-cli-e2e-performance-v2"
    }
  });
  if (!res.ok) {
...
async function fetchStackExchangeTopAccepted({ site, maxItems, cacheFile }) {
  ...
  // Keep fetching top-voted questions until we have enough with accepted answers.
...
async function downloadToFile({ url, outPath }) {
  const res = await fetch(url, { headers: { "user-agent": "mem-cli-e2e-performance-v2" } });
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`HTTP ${res.status} ${res.statusText} for ${url}\n${body.slice(0, 300)}`);
  }
...
async function ensureMovieLensExtracted({ cacheDir }) {
  ...
  if (!exists(zipPath)) {
    ...
    await downloadToFile({ url, outPath: zipPath });
  }
...
async function loadMovieLensPreferenceDataset({ cacheDir, maxDocs, maxQueries, seed }) {
  ...
  assert.ok(
    idxUserId >= 0 && idxRatingMovieId >= 0 && idxRating >= 0 && idxTs >= 0,
    "ratings.csv header mismatch"
  );
  ...
  const docs = picked.map((r) => {
    ...
    const sentiment = r.rating >= 4 ? "liked" : r.rating <= 2 ? "disliked" : "felt neutral about";
...
async function main() {
  ...
  const cacheDir =
    process.env.MEM_CLI_E2E_PERF_V2_CACHE_DIR ||
    process.env.MEM_CLI_E2E_PERF_CACHE_DIR ||
    path.join(root, ".cache", "e2e-performance-v2");
  ...
  assert.ok(modelPath.trim(), "MEM_CLI_MODEL is required (set MEM_CLI_MODEL or run via scripts/e2e-performance-v2.sh)");

  const cliPath = process.env.MEM_CLI_E2E_CLI || path.join(root, "dist", "index.js");
  if (!exists(cliPath)) {
    throw new Error(`CLI entrypoint not found: ${cliPath}\nRun: npm run build`);
  }

  const prevHome = process.env.HOME;
  const homeDir = mkdtemp("mem-cli-perf2-home-");
  const tmpRoot = mkdtemp("mem-cli-perf2-ws-");

  process.env.HOME = homeDir;

  const daemonEnabled = process.env.MEM_CLI_DAEMON !== "0";
  const daemonIdleMs = process.env.MEM_CLI_DAEMON_IDLE_MS || "1800000";
  ...
  try {
    ...
    const baseCliEnv = {
      MEM_CLI_DAEMON: daemonEnabled ? "1" : "0",
      MEM_CLI_DAEMON_IDLE_MS: daemonIdleMs
    };
    ...
    for (const ds of datasets) {
      const wsRoot = path.join(tmpRoot, ds.id);
      fs.mkdirSync(wsRoot, { recursive: true });

      const tokenSafe = String(ds.id).replace(/[^a-zA-Z0-9-]+/g, "-");
      const token = `perf-${tokenSafe}-${seed}`;

      const initRes = runCli({
        cliPath,
        homeDir,
        args: ["init", "--token", token, "--path", wsRoot, "--json"],
        env: baseCliEnv
      });
      ...
      assert.ok(workspacePath, "expected init to return workspace path");

      const memoryDir = path.join(workspacePath, "memory", `corpus-${ds.id}`);
      ...
      const indexRes = runCli({
        cliPath,
        homeDir,
        args: ["reindex", "--token", token],
        env: baseCliEnv
      });
      assert.equal(indexRes.status, 0, indexRes.stderr || indexRes.stdout);
      const indexMs = indexRes.elapsedMs;

      const stateRes = runCli({
        cliPath,
        homeDir,
        args: ["state", "--token", token, "--json"],
        env: { ...baseCliEnv, MEM_CLI_DAEMON: "0" }
      });
      assert.equal(stateRes.status, 0, stateRes.stderr || stateRes.stdout);
      const state = readJsonStdout(stateRes);
      const chunkCount = Number(state.indexChunks) || 0;
      ...
      for (const q of queries) {
        const searchRes = runCli({
          cliPath,
          homeDir,
          args: ["search", q.text, "--token", token, "--json"],
          env: baseCliEnv
        });
        assert.equal(searchRes.status, 0, searchRes.stderr || searchRes.stdout);
        const qMs = searchRes.elapsedMs;
        queryTimes.push(qMs);
        allQueryTimes.push(qMs);

        const out = readJsonStdout(searchRes);
    ...
    console.log("");
    console.log("[mem-cli] e2e performance v2 (daemon end-to-end)");
    console.log(`model: ${modelPath}`);
    console.log(`seed: ${seed}`);
    console.log(`cache: ${cacheDir}`);
    console.log(`daemon: ${daemonEnabled ? "enabled" : "disabled"}`);
    console.log("");
    ...
    const outPath = process.env.MEM_CLI_E2E_PERF_V2_OUT_JSON || path.join(cacheDir, "last-run.json");
    ...
    try {
      runCli({ cliPath, homeDir, args: ["__daemon", "--shutdown"], env: { MEM_CLI_DAEMON: "0" } });
    } catch {}
